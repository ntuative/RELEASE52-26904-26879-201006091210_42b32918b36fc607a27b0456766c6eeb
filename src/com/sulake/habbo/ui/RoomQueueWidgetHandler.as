package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_448:
               ...
               switch(_loc2_.queueSetTarget)
               {
                  case RoomSessionQueueEvent.const_1029:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_891:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_967) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_967) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1176) + 1;
                  }
               }
               else
               {
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            ...
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_891);
               break;
            case RoomWidgetRoomQueueMessage.const_675:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_1029);
               break;
            case RoomWidgetRoomQueueMessage.const_565:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_241,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_448];
      }
