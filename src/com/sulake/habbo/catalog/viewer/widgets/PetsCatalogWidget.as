package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IAvatarImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(_selectedColorIndex >= var_321.length)
         {
            return "";
         }
         var petColor:IPetColor = var_321[_selectedColorIndex] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_630[var_1602] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(_selectedColorIndex < 0 || _selectedColorIndex > var_321.length)
         {
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_119,onTextWindowEvent);
         var_596 = new Map();
         ...
         var_944 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_321 = [];
         var_630 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_944,this);
               if(_loc11_)
               {
                  var_630 = _loc11_.breeds;
                  var_321 = _loc11_.colors;
                  if(var_321 == null)
                  {
                     var_321 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_630.length)
         {
            _loc12_ = var_630[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_944,_loc12_.id),getRaceLocalizationKey(var_944,_loc12_.id)));
            _loc6_++;
         }
         var_596.add(var_944,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_839,onApproveNameResult);
      ...
      private function updateImage() : void
      {
         ...
         if(_selectedColorIndex >= 0 && _selectedColorIndex < var_321.length)
         {
            _loc7_ = var_321[_selectedColorIndex] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_630[var_1602] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_944,_loc3_.id,_loc2_,AvatarScaleType.const_56,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_272,AvatarAction.const_546);
