package com.sulake.core.runtime
{
   ...
   public final class CoreComponent extends ComponentContext implements ICore
   {
      ...
      private static const const_1164:String = "library";
      
      private static const const_1162:String = "asset-libraries";
      
      private static const const_1161:String = "library";
      
      private static const const_1159:String = "service-libraries";
      
      private static const const_1166:String = "library";
      
      private static const const_1160:String = "component-libraries";
       
      
      private var var_791:IEventDispatcher;
      
      private var var_967:Profiler;
      
      private var var_287:uint;
      
      private var var_550:Array;
      
      private var var_288:LibraryLoaderQueue;
      ...
      public function CoreComponent(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_662 = (param2 & 0) == Core.CORE_SETUP_DEBUG;
         var_550 = new Array();
         var_649 = new Array();
         var_217 = param1;
         var _loc3_:int = 0;
         ...
         switch(param2 & 0)
         {
            ...
            case Core.const_989:
               ...
               break;
            case Core.const_1426:
               debug("Core; using profiler frame update handler");
               var_217.addEventListener(Event.ENTER_FRAME,profilerFrameUpdateHandler);
               var_967 = new Profiler(this);
               attachComponent(var_967,[new IIDProfiler()]);
               var_792 = new ProfilerViewer();
               var_217.addChild(var_792);
               break;
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1319);
         if(!_isDisposed)
         {
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         if(var_288 == null)
         {
            var_288 = new LibraryLoaderQueue(var_662);
         }
         try
         {
            ...
            if(node != null)
            {
               ...
               for each(item in list)
               {
                  ...
                  loader = new ZipLoader(context,true,var_662);
         ...
         {
            error("Failed to parse zip libraries from config xml!",true,Core.const_175,e);
         }
         try
         {
            node = config.child(const_1162)[0];
            if(node != null)
            {
               list = node.child(const_1161);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,var_662);
                  assets.loadFromFile(loader,true);
                  loader.load(new URLRequest(url));
                  var_288.push(loader);
                  loader.addEventListener(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,updateLoadingProcess);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_175,e);
         }
         try
         {
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_175,e);
         }
         try
         {
            node = config.child(const_1160)[0];
            if(node != null)
            {
               list = node.child(const_1164);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,var_662);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_175,e);
         }
         if(!disposed)
         {
         ...
         while(criticality < CoreComponent.const_144)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     var_967.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
      ...
      private function complexFrameUpdateHandler(param1:Event) : void
      {
         ...
         while(criticality < CoreComponent.const_144)
         {
            ...
            {
               if(var_649[criticality] < criticality)
               {
                  ++0;
                  skip = true;
               }
      ...
      private function updateLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(var_791)
               {
                  var_791.dispatchEvent(new LibraryProgressEvent(_loc2_.url,var_247 - 0,var_247,_loc2_.elapsedTime));
               }
            }
         }
         if(false)
