package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1085:uint = 1;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_783)
            {
               if(var_696 != null)
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ": " + var_696.message,var_696.errorID,var_696);
               }
               else
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ". No error data available!",0,var_696);
               }
            }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1085) : uint(const_518);
      }
